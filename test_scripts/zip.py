...
from statsmodels.distributions.discrete import zipoisson
...
# Simulate model data
def zip_simulator(mu_params, w_params, N):
...
N = 100000
nparams = 5
mu_realparams = np.array([0.011, 0.015, 0.013, 0.008, 0.005])
w_realparams = np.array([0.0010, 0.0020, 0.0033, 0.0043, 0.0012])
X, Y = zip_simulator(
    mu_realparams,
    w_realparams,
    N
...
# Fitted model
out = cm.ZeroInflatedPoisson(Y, X, exog_infl=X)
res = out.fit()
